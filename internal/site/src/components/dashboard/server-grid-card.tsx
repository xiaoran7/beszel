...
import { formatBytes, secondsToUptimeString } from "@/lib/utils"
...
export const ServerGridCard = memo(function ServerGridCard({ system }: ServerGridCardProps) {
	...
	const cpu = isOnline ? (system.info?.cpu_percent ?? system.info?.c_p ?? 0) : 0
	...
	const network = useMemo(() => {
		...
		if (system.info?.b) {
			return `${(system.info.b / 1024 / 1024).toFixed(0)} Mbps`
		}
		return "—"
	...
	const sparklineData = useMemo(() => {
		if (!isOnline) return []
		const base = typeof cpu === "number" ? cpu : 0
		// Smooth visual pulse reflecting load variations
		return [
			Math.max(0, base * 0.9),
			Math.max(0, base * 1.05),
			Math.max(0, base * 0.95),
			Math.max(0, base * 1.1),
			Math.max(0, base * 0.9),
			Math.max(0, base * 1.15),
				...
				<div className="flex items-start justify-between gap-2">
					<div className="flex items-center gap-2.5 min-w-0 flex-1">
						...
						<div className="flex flex-col min-w-0 flex-1">
							...
							<span className="text-[11px] text-muted-foreground font-medium truncate" title={subtitle}>
								{subtitle}
							</span>
						</div>
					</div>
